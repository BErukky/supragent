...
def _fetch_single_feed(url, config):
    """Fetch and parse one RSS feed, returning a list of structured news items."""
    try:
...
def fetch_headlines():
    ...
    with ThreadPoolExecutor(max_workers=3) as executor:
        futures = [
            executor.submit(_fetch_single_feed, url, config)
        ...
        for future in futures:
            try:
                ...
                print(f"Error collecting feed result: {e}")
